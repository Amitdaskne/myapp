...
bool ImGui::tab(const char* label, bool selected)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = CalcItemSize(ImVec2(65, 40), 50 + style.FramePadding.x, label_size.y + style.FramePadding.y);
    const ImRect bb(pos, pos + size);
    ItemSize(size, style.FramePadding.y);
    if (!ItemAdd(bb, id))
        return false;
    ...
    int a = int(t * 255);
    auto text_color = ImGui::GetColorU32(ImLerp(ImVec4(180 / 255.f, 180 / 255.f, 180 / 255.f, 180 / 255.f), ImVec4(accentcolor), t));
    draw->AddRectFilled(ImVec2(bb.Min.x, bb.Max.y - 2), bb.Max, ImGui::GetColorU32(ImLerp(ImVec4(0,0,0,0), ImVec4(accentcolor),t)));
    draw->AddText(ImVec2(pos.x + 78 / 2 - label_size.x / 2, pos.y + 22 - label_size.y / 2), text_color, label);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.gradient_color0 = ImLerp(it_anim->second.gradient_color0, *v ? c::checkbox_gradient_start : hovered ? ImVec4(0, 0, 0, 0) : ImVec4(0, 0, 0, 0), g.IO.DeltaTime * 6.f);
    it_anim->second.gradient_color1 = ImLerp(it_anim->second.gradient_color1, *v ? c::checkbox_gradient_end : hovered ? ImVec4(0, 0, 0, 0) : ImVec4(0, 0, 0, 0), g.IO.DeltaTime * 6.f);
    it_anim->second.checkmark_color = ImLerp(it_anim->second.checkmark_color, *v ? c::checkbox_mark : hovered ? ImVec4(0, 0, 0, 0) : ImVec4(0, 0, 0, 0), g.IO.DeltaTime * 9.f);
    it_anim->second.rg = ImLerp(it_anim->second.rg, *v ? 0.f : 10.f, g.IO.DeltaTime * 5.f);


    it_anim->second.text_color = ImLerp(it_anim->second.text_color, *v ? c::text_active : hovered ? c::text_hov : c::text, g.IO.DeltaTime * 6.f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));
    RenderNavHighlight(total_bb, id);

    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(c::checkbox_bg), true, style.FrameRounding);

    const int vtx_idx_1 = ImGui::GetWindowDrawList()->VtxBuffer.Size;
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(it_anim->second.rg, it_anim->second.rg), check_bb.Max - ImVec2(it_anim->second.rg, it_anim->second.rg), GetColorU32(it_anim->second.gradient_color0), style.FrameRounding);
    ImGui::ShadeVertsLinearColorGradientKeepAlpha(ImGui::GetWindowDrawList(), vtx_idx_1, ImGui::GetWindowDrawList()->VtxBuffer.Size, check_bb.Min, check_bb.Max, GetColorU32(it_anim->second.gradient_color1), GetColorU32(it_anim->second.gradient_color0));

    const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
    RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad + 3, pad + 3 + it_anim->second.rg), GetColorU32(it_anim->second.checkmark_color), square_sz - pad * 4.0f);

    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y);

    window->DrawList->AddText(label_pos, GetColorU32(it_anim->second.text_color), label);
